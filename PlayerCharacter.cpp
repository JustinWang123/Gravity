...
// ------------------------------------------------------------------------------------------------
PlayerCharacter :: PlayerCharacter(Uint32 setId, GameSim* setGameSim)
	:	attackDelay(0),
		id(setId),
		isActive(false),
		gameSim(setGameSim) {
...
void PlayerCharacter :: Draw(Vector2df camPos) const {
	...
	if(!IsDead()) {
		...
		// Draw aimmer:
		DrawSurface(
			charScreenPos.x + (GetHeading().x * AIMMER_DISTANCE) - 1,
			charScreenPos.y + (GetHeading().y * AIMMER_DISTANCE) - 1,
			surfaceHeading);
...
void PlayerCharacter :: UpdateVelocity(float timeDelta) {
	...
	vel += gameSim->GetPosGravity(pos) * timeDelta;
...
void PlayerCharacter :: UpdatePosition(float timeDelta) {
	...
	if(vel.x > 0) {
		if(!gameSim->CheckCollisionWithLevel( Vector2df(charRight + vel.x * timeDelta, charTop + 2))
		&& !gameSim->CheckCollisionWithLevel( Vector2df(charRight + vel.x * timeDelta, charBottem - 2))) {
	...
	// Handle velocity to the left:
		...
		if(!gameSim->CheckCollisionWithLevel(Vector2df(charLeft + vel.x * timeDelta, charTop + 2))
		&& !gameSim->CheckCollisionWithLevel(Vector2df(charLeft + vel.x * timeDelta, charBottem - 2))) {
			pos.x += vel.x * timeDelta;
		}
		else {
			vel.x = 0;
	...
	if(vel.y > 0) {
		if(!gameSim->CheckCollisionWithLevel(Vector2df(charRight - 2, charBottem + vel.y * timeDelta))
		&& !gameSim->CheckCollisionWithLevel(Vector2df(charLeft + 2, charBottem + vel.y * timeDelta))) {
	...
	// Handle velocity up
		...
		if(!gameSim->CheckCollisionWithLevel(Vector2df(charRight - 2, charTop + vel.y * timeDelta))
		&& !gameSim->CheckCollisionWithLevel(Vector2df(charLeft + 2, charTop + vel.y * timeDelta))) {
			pos.y += vel.y;
		}
		else {
			vel.y = 0;
...
void PlayerCharacter :: UpdateCollision(float timeDelta) {
	...
	if(gameSim->CheckCollisionWithLevel( Vector2df(charLeft + 2, charTop - 2))
	|| gameSim->CheckCollisionWithLevel( Vector2df(charRight - 2, charTop - 2))) {
		collisionAbove = true;
	}
	else {
	...
	if(gameSim->CheckCollisionWithLevel( Vector2df(charLeft + 2, charBottem + 2))
	|| gameSim->CheckCollisionWithLevel( Vector2df(charRight - 2, charBottem + 2))) {
		collisionBelow = true;
	}
	else {
	...
	if(gameSim->CheckCollisionWithLevel( Vector2df(charLeft - 2, charBottem - 2))
	|| gameSim->CheckCollisionWithLevel( Vector2df(charLeft - 2, charTop + 2))) {
		collisionLeft = true;
	}
	else {
		collisionLeft = false;
	...
	if(gameSim->CheckCollisionWithLevel( Vector2df(charRight + 2, charBottem - 2))
	|| gameSim->CheckCollisionWithLevel( Vector2df(charRight + 2, charTop + 2))) {
		collisionRight = true;
	}
	else {
...
void PlayerCharacter :: UpdateFriction(float timeDelta) {
	if( (collisionAbove || collisionBelow) && !gameSim->IsPosGravityWell(pos) ){
		vel.x *= powf(CHARACTER_FRICTION, timeDelta);
	}

	if( (collisionLeft || collisionRight) && !gameSim->IsPosGravityWell(pos) ){
...
void PlayerCharacter :: UpdateSmokeTrail(float timeDelta) {
	...
	if(keysDown[SDLK_w] || keysDown[SDLK_s] || keysDown[SDLK_a] || keysDown[SDLK_d]) {
		if(makeSmokeTime <= 0) {
			gameSim->CreateSmokeParticle(id, pos - vel.Normal() * 4.0f, controlState.headingDeg - 180, 0.5f);
...
// ------------------------------------------------------------------------------------------------
Vector2df PlayerCharacter :: GetHeading() const {
...
void PlayerCharacter :: Attack(ProjectileType type) {
	if(IsReadyToAttack()) {
		if(type == PROJECTILE_TYPE_BULLET && numOfBullets > 0) {
			gameSim->CreateBullet(id, pos + GetHeading() * CHARACTER_RADIUS, GetHeading());
			numOfBullets--;
		}
		else if(type == PROJECTILE_TYPE_MISSILE && numOfMissiles > 0) {
			gameSim->CreateMissile(id, pos + GetHeading() * CHARACTER_RADIUS, GetHeading());
			numOfMissiles--;
		}
		else if(type == PROJECTILE_TYPE_MINE && numOfMines > 0) {
			gameSim->CreateMine(id, pos + GetHeading() * CHARACTER_RADIUS,  GetHeading());
			numOfMines--;
		}
