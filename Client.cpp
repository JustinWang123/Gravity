	...
	*/
	...
	for(Uint32 i = 0; i < MAX_PLAYERS; i++) {
		if(players[i].state != PLAYER_STATE_DISCONECTED) {
			...
			str += " - " + ToString(gameSim->GetPlayerCharacterScore(i));
	...
	while ( n-- > 0 ) {
		...
		switch (packets[n]->data[0]) {
			...
			case PACKET_UPDATE_WORLD:
				gameSim->HandleUpdateWorld(packets[n]);
				break;

			case PACKET_LEVEL_DATA:
				gameSim->HandleLevelData(packets[n]);
				break;
...
void Client :: HandleConnectionAccepted(UDPpacket* packet) {
	...
	// Get pc:
	gameSim->MakeCharacterControllable(myChannel);
	myPc = gameSim->GetPlayerCharacter(myChannel);
